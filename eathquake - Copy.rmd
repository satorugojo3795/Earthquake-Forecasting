...
ecdf_values <- ecdf(aquila.bru.copy$consecutive_difference)

# Plot the ECDF
ggplot(data = aquila.bru.copy, aes(x = consecutive_difference)) +
  stat_ecdf() +
...
# Assuming best_dfs_copy is your list of dataframes
...
```{r}
plot_cdf(full_data = aquila.bru.copy,
         data_name = "HORUS data")
```

```{r}
library(gridExtra)
# Apply the plot_cdf function to each dataframe in best_dfs_copy
plots <- lapply(seq_along(best_dfs_copy), function(i) {
  df <- best_dfs_copy[[i]]
  data_name <- paste("synthetic data", i)
  plot_cdf(full_data = df, data_name = data_name)
})

# Arrange the plots as subplots
grid.arrange(grobs = plots, ncol = 2, width = 20, height = 10)  # Adjust width and height as needed
```

```{r}
plot_cdf(full_data = best_dfs_copy[[1]],
         data_name = "synthetic dataset 1")

```

```{r}
# Plot ECDF for aquila.bru.copy
plot_aquila_bru <- plot_cdf(aquila.bru.copy, data_name = "aquila.bru.copy")

# Plot ECDF for each dataset in best_dfs_copy
plot_best_dfs <- lapply(seq_along(best_dfs_copy), function(i) {
  df <- best_dfs_copy[[i]]
  data_name <- paste("Dataset", i)
  plot_cdf(full_data = df, data_name = data_name)
})

# Combine all plots using patchwork
library(patchwork)
all_plots <- plot_aquila_bru + wrap_plots(plot_best_dfs, ncol = 2)
...
all_names <- c("aquila.bru.copy", sapply(seq_along(best_dfs_copy), function(i) paste("Dataset", i)))
