...
Before going further we will generate 100 catalogues; 50 of these will not have any imposed events while the rest will have imposed events. We then fit an ETAS model all of these dataset and see how close are the estimates of the parameters are to the true parameters. For this we will create a dataframe $catalogues_characteristics$, where we will store the the estimates of our parameters and 
also store the optimal number of clusters

```{r}
catalogues_characteristics <- data.frame(matrix(0, nrow = 20, ncol = 8))
...
num_observations <- numeric(length(synthetic_dataset_all))
...
obs_class <- cut(num_observations, breaks = c(100,150,200,250,300),
...
for (class_label in unique_classes) {
  ...
  if (length(class_indices) >= datasets_per_class) {
    ...
    selected_datasets <- c(selected_synthetic_datasets,
