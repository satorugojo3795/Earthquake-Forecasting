...
ggplot() +
  geom_sf(data = df_cat.sf[df_cat$M > 3 & df_cat$M < 4.5,], aes(color = "Small Earthquakes"), size = 0.05) +
  geom_sf(data = italy.map, fill = alpha("lightgrey", 0), color = 'green', linewidth = 0.7, aes(fill = "Italy Map")) +
  geom_sf(data = df_cat.sf[df_cat$M >= 4.5,], aes(color = "Large Earthquakes"), size = 0.5) +
  scale_color_manual(values = c("Small Earthquakes" = "black", "Large Earthquakes" = "orange"),
                     labels = c("Small Earthquakes" = "Small earthquakes with magnitude greater than 3", "Large Earthquakes" = "Large earthquakes")) +
  scale_fill_manual(values = c("Italy Map" = "lightgrey")) +
  labs(title = "Map of Event Locations", color = "Legend") +
  theme_minimal() +
...
# aquila.bru.copy <- subset(aquila.bru.copy, select = -idx.p)
...
all_names <- c("Horus catalogue", sapply(seq_along(best_dfs_copy), function(i) paste("Synthetic Dataset", i)))
